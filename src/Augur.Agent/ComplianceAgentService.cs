...
/// <summary>Runs <see cref="ComplianceAgent"/> and persists produced findings via <see cref="IUnitOfWork"/>.</summary>
public sealed class ComplianceAgentService
{
    ...
    public async Task<AgentResult> RunAndPersistAsync(AgentRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var f in result.Findings)
        {
            ...
            var finding = Finding.Create(f.Title, f.Description, f.Severity, request.Source, f.ExternalId);
